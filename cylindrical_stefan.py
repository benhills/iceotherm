...
class cylindrical_stefan():
    def __init__(self,const=const):
        ...
        self.Tinf = -15.                        # Far Field Temperature
        self.Tm = 0.                            # Pure Melting Temperature
        self.Q = 0.0                            # Heat Source after Melting (W)
        self.Qmelt = 2500.                      # Heat Source for Melting (W)
        self.Cinit = 0.0                        # Initial Solute Concentration (before injection)
        self.Cinject = 0.2*const.rhoe           # Injection Solute Concentration

        # Domain Specifications
        self.Rcenter = 0.01                     # Innner Domain Edge
        self.Rinf = 1.                          # Outer Domain Edge
        self.R_melt = 0.04                          # Melt-Out Radius
        self.n = 100
        self.rs = np.linspace(self.Rcenter,self.Rinf,self.n)
        self.dt = 10.
        self.t_final = 2.*3600.
    ...
    def log_transform(self,const=const):
        ...
        self.Tstar = self.Tinf/abs(self.Tinf)
        self.Rstar = self.R_melt/self.R_melt
        self.Rstar_center = self.Rcenter/self.R_melt
        self.Rstar_inf = self.Rinf/self.R_melt
        self.Qstar = self.Q/(2.*np.pi*const.ki*abs(self.Tinf))
        self.astar_i = const.L*const.rhoi/(const.rhoi*const.ci*abs(self.Tinf))
        self.t0 = const.rhoi*const.ci/const.ki*self.astar_i*self.Rcenter**2.
    ...
    def get_domain(self):
        ...
        if 'solve_sol_temp' in self.flags or 'solve_sol_mol' in self.flags:
            self.sol_mesh = dolfin.IntervalMesh(self.n,self.Rcenter,self.Rstar)
    ...
    def get_initial_conditions(self):
        ...
        T,lam,self.R_melt,self.t_melt = analyticalMelt(np.exp(self.ice_coords[:,0])*self.R_melt,self.Tinf,self.Qmelt,R_target=self.R_melt)
        self.u0_i.vector()[:] = T[:]/abs(self.Tinf)
        if 'solve_sol_temp' in self.flags:
            # s (solution temperature),
            self.u0_s = dolfin.Function(self.sol_V)
            T,lam,self.R_melt,self.t_melt = analyticalMelt(np.exp(self.sol_coords[:,0])*self.R_melt,self.Tinf,self.Qmelt,R_target=self.R_melt)
            self.u0_s.vector()[:] = T[:]/abs(self.Tinf)
        if 'solve_sol_mol' in self.flags:
            # c (solution concentration),
            self.u0_c = dolfin.interpolate(dolfin.Constant(self.Cinit),self.sol_V)
    ...
    def solve_molecular(self,t):
        ...
        if abs(t*self.t0-self.t_inject) < const.tol:
            ...
            self.inject_mass = self.C_inject*(np.exp(self.Rstar)**2.-np.exp(self.Rcenter)**2.)
            print('Inject!')
    ...
    def solve_thermal(self):
            ...
            #F_i -= dt*(Qsource*t0/abs(Tinf)/(const.rhoi*const.ci))*v_i*dx
            a_i = dolfin.lhs(F_i)
            L_i = dolfin.rhs(F_i)
            ...
            #F_s -= dt*(Qsource*t0/abs(Tinf)/(const.rhow*const.cw))*v_s*dx
            # Center heat flux
            F_s -= (self.Qwater/(const.kw*diff_ratio*2.*np.pi*abs(self.Tinf)))*self.v_s*self.sds(2)
            a_s = dolfin.lhs(F_s)
            L_s = dolfin.rhs(F_s)
    ...
    def move_wall(self):
        ...
        Frozen = np.exp(self.ice_coords[:,0])[self.ice_idx_wall]+self.dR < 0.
        if Frozen:
        ...
        if 'solve_sol_temp' in self.flags:
            # --- Move solution mesh (both for temperature and solute) --- #
            # stretch mesh rather than uniform displacement
            ...
            dolfin.ALE.move(self.sol_mesh,dolfin.Expression('std::log(exp(x[0])+dRs*(exp(x[0])-Rstar_center))-x[0]',dRs=self.dR/(self.Rstar-self.Rstar_center),Rstar_center=self.Rstar_center))
            self.sol_mesh.bounding_box_tree().build(self.sol_mesh)

        self.Rstar = np.exp(self.ice_coords[self.ice_idx_wall])
    ...
    def run(self):
        ...
        self.R_result = [np.exp(self.ice_coords[:,0])*self.R_melt]
        self.T_result = [np.array(self.u0_i.vector()[:]*abs(self.Tinf))]
        for t in self.ts[1:]:
            print(t*self.t0/60.)
            ...
            if t in self.save_times:
                self.R_result = np.append(self.R_result,[np.exp(self.ice_coords[:,0])*self.R_melt],axis=0)
                self.T_result = np.append(self.T_result,[self.u0_i.vector()[:]*abs(self.Tinf)],axis=0)
